...
//	}
		try {
			...
			// Construct the forecast object
			JSONObject forecastObject = forecastData.getJSONObject("forecast");
			double elevation = forecastObject.getDouble("elevation");
			double generationTimeMs = forecastObject.getDouble("generationtime_ms");
			double latitude = forecastObject.getDouble("latitude");
			double longitude = forecastObject.getDouble("longitude");
			String timezone = forecastObject.getString("timezone");
			String timezoneAbbreviation = forecastObject.getString("timezone_abbreviation");
			int utcOffsetSeconds = forecastObject.getInt("utc_offset_seconds");
			...
			itemAttributes.put("id", new AttributeValue().withS(id));
			itemAttributes.put("forecast", new AttributeValue().withM(createForecastMap(elevation, generationTimeMs, latitude, longitude, timezone, timezoneAbbreviation, utcOffsetSeconds, timeArray, temperatureArray)));

			// Put the item into the DynamoDB table
			dynamoDB.putItem(dynamoDBTableName, itemAttributes);
		} catch (JSONException e) {
			e.printStackTrace();
			// Handle JSON parsing error
		}
	...
	private Map<String, AttributeValue> createForecastMap(double elevation, double generationTimeMs, double latitude, double longitude, String timezone, String timezoneAbbreviation, int utcOffsetSeconds, JSONArray timeArray, JSONArray temperatureArray) throws JSONException {
		Map<String, AttributeValue> forecastMap = new HashMap<>();
		forecastMap.put("elevation", new AttributeValue().withN(String.valueOf(elevation)));
		forecastMap.put("generationtime_ms", new AttributeValue().withN(String.valueOf(generationTimeMs)));
		forecastMap.put("latitude", new AttributeValue().withN(String.valueOf(latitude)));
		forecastMap.put("longitude", new AttributeValue().withN(String.valueOf(longitude)));
		forecastMap.put("timezone", new AttributeValue(timezone));
		forecastMap.put("timezone_abbreviation", new AttributeValue(timezoneAbbreviation));
		forecastMap.put("utc_offset_seconds", new AttributeValue().withN(String.valueOf(utcOffsetSeconds)));

		// Construct the hourly object
		Map<String, AttributeValue> hourlyMap = new HashMap<>();
		hourlyMap.put("time", new AttributeValue().withL(createStringList(timeArray)));
		hourlyMap.put("temperature_2m", new AttributeValue().withL(createNumberList(temperatureArray)));
