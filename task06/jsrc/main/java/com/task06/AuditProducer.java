...
import com.task06.model.ConfigurationItem;
...
public class AuditProducer implements RequestHandler<DynamodbEvent, Void> {
	...
	private void handleModifyEvent(DynamodbEvent.DynamodbStreamRecord record) {
		...
		String logMessage = "Audit entry for MODIFY event: " + putItemRequest;
		logger.log(Level.INFO, logMessage);
	}

	private ConfigurationItem convertDynamodbRecordToConfigurationItem(Map<String, AttributeValue> dynamodbImage) {
	...
	private AuditEntry createAuditEntry(ConfigurationItem newItem) {
		...
		auditEntry.setModificationTime(LocalDateTime.parse(LocalDateTime.now().format(ISO_DATE_TIME_FORMATTER)));
